...
DELETE FROM CUSTOMIZED.MAINTENANCE_OF_INDEX WHERE EXECUTED_DATE < DATEADD(DAY, -30, GETDATE());
	...
	FROM   SYS.Dm_db_index_physical_stats(Db_id(), NULL, NULL, NULL, NULL) INDEXSTATS 
		   ...
		   WHERE  INDEXSTATS.AVG_FRAGMENTATION_IN_PERCENT > 5  and IND.NAME is not null order by INDEXSTATS.AVG_FRAGMENTATION_IN_PERCENT desc
       
	INSERT INTO customized.MAINTENANCE_OF_INDEX  (DATABASE__NAME,SCHEMA__NAME,TABLE_NAME,INDEX_NAME,INDEX_TYPE,AVG_GRAG,TASK_PERFORMED,TASK_STATUS,EXECUTED_DATE)
	SELECT database_name,schemaname,TableName, IndexName,IndexType,avg_fragmentation_in_percent,NULL,NULL,GETDATE() FROM #INDEX 

	UPDATE customized.MAINTENANCE_OF_INDEX SET TASK_PERFORMED='USE ['+DATABASE__NAME+'];ALTER INDEX ['+INDEX_NAME+'] ON '+QUOTENAME(SCHEMA__NAME)+'.'+QUOTENAME(TABLE_NAME)+' REORGANIZE;' WHERE convert(float,avg_grag)<20
	UPDATE customized.MAINTENANCE_OF_INDEX SET TASK_PERFORMED='USE ['+DATABASE__NAME+'];ALTER INDEX ['+INDEX_NAME+'] ON '+QUOTENAME(SCHEMA__NAME)+'.'+QUOTENAME(TABLE_NAME)+' REBUILD;' WHERE convert(float,avg_grag)>20
...
/*
----------------- managed locked indexes


IF EXISTS(SELECT COUNT(1) FROM customized.MAINTENANCE_OF_INDEX where TASK_STATUS = 'LOCKED')
BEGIN
--UPDATE CUSTOMIZED.MAINTENANCE_OF_INDEX SET TASK_PERFORMED=REPLACE(REPLACE(TASK_PERFORMED,'REBUILD;','REBUILD WITH (ONLINE = ON);'),'REORGANIZE;','REORGANIZE WITH (ONLINE = ON);') WHERE TASK_STATUS = 'LOCKED' -- AND TASK_PERFORMED NOT LIKE '%WITH (ONLINE = ON);%'

DECLARE @TASK VARCHAR(MAX)

DECLARE C CURSOR FOR 
select DISTINCT TASK_PERFORMED from customized.MAINTENANCE_OF_INDEX where TASK_STATUS IN('FAILED','LOCKED')
OPEN C 
FETCH NEXT FROM C INTO @TASK
WHILE @@FETCH_STATUS = 0
BEGIN

	UPDATE CUSTOMIZED.MAINTENANCE_OF_INDEX SET TASK_STATUS='STARTED' WHERE TASK_PERFORMED=@TASK
		PRINT (@TASK);
	BEGIN TRY
		EXEC (@TASK);
		UPDATE CUSTOMIZED.MAINTENANCE_OF_INDEX SET TASK_STATUS='COMPLETED'  WHERE TASK_PERFORMED=@TASK
	END TRY
	BEGIN CATCH
		UPDATE CUSTOMIZED.MAINTENANCE_OF_INDEX SET TASK_STATUS='FAILED'  WHERE TASK_PERFORMED=@TASK
	END CATCH
